...
async function handleRequest(request, event) {
  ...
  try {
    ...
    if (!targetUrl) {
      return createErrorResponse('Missing URL parameter. Usage: ?url=encoded_url or /m3u8filter/encoded_url', 400);
    }
    ...
    // 5. 创建缓存键并检查缓存
    const cacheKey = await createCacheKey(request, targetUrl);
    ...
    // 9. 创建响应并使用gzip压缩(如果启用)
    const response = await createM3U8Response(processedContent, targetResponse.headers);
    ...
    return createErrorResponse(`Error processing request: ${error.message}`, 500);
  }
}
 ...
 * @returns {Promise<Request>} 缓存键
 */
async function createCacheKey(request, targetUrl) {
  ...
  cacheUrl.pathname = `/cache/${urlHash}`;
 ...
 * @returns {Promise<Response>} 响应
 */
async function createM3U8Response(content, originalHeaders) {
  ...
  // 保留必要的原始头
  if (CONFIG.PRESERVE_ORIGINAL_HEADERS && originalHeaders) {
    ['Content-Disposition', 'Expires', 'Last-Modified'].forEach(key => {
      if (originalHeaders.has(key)) {
        headers.set(key, originalHeaders.get(key));
      }
    });
  }
